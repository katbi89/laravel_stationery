    ...
    <div class="content-wrapper">
        ...
        <section class="content-header">
            <h1>
                ...
                <small>{{ __('All Customer Payments') }}</small>
            </h1>
            <ol class="breadcrumb">
                <li>
                    <a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ __('Dashboard') }}</a>
                </li>
                <li class="active">{{ __('Customer Payments') }}</li>
            </ol>
        </section>
                ...
                <div class="col-xs-12">
                    <div class="box">
                        <div class="box-header">
                            <h3 class="box-title">
                                {{ link_to_route('dashboard.customerPayments.create', __('Create New Payment'), null, ['class' => 'btn btn-success']) }}
                            </h3>
                        </div>
                        <!-- /.box-header -->
                        <div class="box-body">
                            <table id="data_table" class="table table-bordered table-hover">
                                ...
                                @foreach($payments as $payment)
                                    <tr>
                                        ...
                                        <td>{{ $payment->notes }}</td>
                                        <td>
                                            <a href="{{ route('dashboard.customerPayments.show', $payment->id) }}"
                                               class="btn btn-box-tool" data-toggle="tooltip" title="{{ __('View') }}">
                                                <i class="fa fa-eye"></i>
                                            </a>
                                            <a href="{{ route('dashboard.customerPayments.edit', $payment->id) }}"
                                               class="btn btn-box-tool" data-toggle="tooltip" title="{{ __('Edit') }}">
                                                <i class="fa fa-pencil-square-o"></i>
                                            </a>
                                            @if(!$payment->deleted_at)
                                                {{ Form::open(['route' => ['dashboard.customerPayments.destroy', $payment->id], 'method' => 'delete', 'class' => 'inline']) }}
                                                ...
                                                {{ Form::close() }}
                                            @else
                                                {{ Form::open(['route' => ['dashboard.customerPayments.restore', 'id' => $payment->id ], 'method' => 'post', 'class' => 'inline']) }}
                                                <button type="submit" class="btn btn-box-tool" data-toggle="tooltip"
                                                        title="{{ __('Restore') }}"
                                                ...
                                                @can('forceDelete', $payment)
                                                    {{ Form::open(['route' => ['dashboard.customerPayments.forceDelete', 'id' => $payment->id ], 'method' => 'post', 'class' => 'inline']) }}
                                                    <button type="submit" class="btn btn-box-tool" data-toggle="tooltip"
