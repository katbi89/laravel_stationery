...
@section('title', __('Create New Supplier Payment'))
                ...
                <div class="col-md-12">
                    ...
                    <div class="box box-default">
                        <div class="box-header with-border">
                            <h3 class="box-title">{{ __('Create New Supplier Payment') }}</h3>
                        </div>
                        <!-- /.box-header -->
                        <!-- form start -->
                        {{ Form::model($payment, ['route' => ['dashboard.supplierPayments.store'], 'method' => 'post', 'autocomplete' => 'off']) }}
                        <div class="box-body">
                            @if(isset($payment->supplier_id))
                                <div class="form-group">
                                    {{ Form::label('supplier_id', __('Supplier') . ':', ['class' => 'control-label']) }}
                                    {{ Form::hidden('supplier_id') }}
                                    <h4>{{ $payment->supplier->company  }}</h4>
                                </div>
                            @else
